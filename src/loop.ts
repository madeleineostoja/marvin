...
function printHeader(config: LoopConfig, harness: Harness): void {
  const artLines = personality.art.active.split("\n");
  const workspaceName =
    config.workspaceRoot.split("/").pop() || config.workspaceRoot;
...
function printSummary(
  iterations: number,
  startTime: number,
  reason: "completed" | "stalled" | "aborted" | "blocked",
): void {
  ...
  const quoteLines = ui.quoteBlock(
    personality.pick(personality.summary[reason]),
  );
...
function printIterationBanner(iteration: number): void {
  ui.blank();
  ui.labeledRule(`Iteration ${iteration}`);
}

function printMainBranchError(): void {
  ui.blank();
  ui.status("red", "Error");
  const quoteLines = ui.quoteBlock(
    personality.pick(personality.errors.mainBranch),
  );
  for (const line of quoteLines) {
    ui.log(line);
  }
...
export function parseExitStatus(output: string): "blocked" | "continue" {
...
export function countIncompleteTasks(planContent: string): number {
...
async function snapshotWorkingTree(cwd: string): Promise<string> {
  ...
  const diff = await execa("git", ["diff", "HEAD"], { cwd }).then(
    (r) => r.stdout,
  );
...
async function isMainBranch(cwd: string): Promise<boolean> {
...
async function acquireLock(
  ...
  try {
    ...
    const existingPid = await readFile(lockPath, "utf-8").catch(
      () => "unknown",
    );
    ...
    const quoteLines = ui.quoteBlock(
      personality.pick(personality.errors.locked),
    );
    for (const line of quoteLines) {
      ui.log(line);
    }
    ui.blank();
    ui.log(
      styleText("dim", `Lock held by PID ${existingPid.trim()} (${LOCK_FILE})`),
    );
...
async function runPreflightCheck(config: LoopConfig): Promise<void> {
  ...
  try {
    ...
    const quoteLines = ui.quoteBlock(
      personality.pick(personality.preflight.passed),
    );
    for (const line of quoteLines) {
      ui.log(line);
    }
    ...
    const quoteLines = ui.quoteBlock(
      personality.pick(personality.preflight.failed),
    );
    for (const line of quoteLines) {
      ui.log(line);
    }
...
export function summarizeToolCall(event: Extract<StreamEvent, { type: "tool" }>): string {
...
function formatEventStatus(event: Extract<StreamEvent, { type: "tool" }>): string | null {
...
export async function runLoop(
  ...
  if (!existsSync(planPath)) {
    ...
    const quoteLines = ui.quoteBlock(
      personality.pick(personality.errors.noPlan),
    );
    for (const line of quoteLines) {
      ui.log(line);
    }
  ...
  try {
    ...
    let exitReason: "completed" | "stalled" | "aborted" | "blocked" =
      "completed";
    ...
    while (ctx.iteration < config.maxIterations) {
      if (signal.aborted) {
        ...
        const quoteLines = ui.quoteBlock(
          personality.pick(personality.shutdown),
        );
        for (const line of quoteLines) {
          ui.log(line);
        }
      ...
      try {
        ...
        function updateSpinnerText(personalityLines: string[]): void {
      ...
      if (ctx.stallCount >= MAX_STALLS) {
        ...
        const quoteLines = ui.quoteBlock(
          personality.pick(personality.summary.stalled),
        );
        for (const line of quoteLines) {
          ui.log(line);
        }

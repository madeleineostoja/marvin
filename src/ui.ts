...
export function rule(width?: number): void {
  const termWidth = process.stdout.columns || 80;
  const w = width ?? termWidth - PAD.length;
  log(styleText("dim", RULE_CHAR.repeat(w)));
}

export function labeledRule(label: string, width?: number): void {
  const termWidth = process.stdout.columns || 80;
...
export function quoteBlock(
  text: string,
  maxWidth: number = MAX_QUOTE_WIDTH,
): string[] {
...
export function dimBox(lines: string[]): string[] {
  if (lines.length === 0) {
    return [];
  }

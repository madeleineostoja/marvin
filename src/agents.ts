...
- Never use \`git add -f\` or otherwise force-stage gitignored files. Respect the ignore list. Plan and spec files are expected to be gitignored — update the plan on disk as normal; \`git add -A\` will skip it (and any spec files), and that is correct.
...
export function claudeAgents(models: ModelsConfig): Record<string, unknown> {
  return {
    ...
    [REVIEW_AGENT]: {
      description:
        "Code review specialist. Reviews diffs for quality, correctness, and acceptance criteria.",
...
export function opencodePermissions(planFile: string): Record<string, unknown> {
  return {
    question: "deny",
    doom_loop: "deny",
